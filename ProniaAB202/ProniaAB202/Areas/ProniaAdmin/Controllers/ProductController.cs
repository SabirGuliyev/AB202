    ...
    public class ProductController : Controller
    {
        ...
        public async Task<IActionResult> Create(CreateProductVM productVM)
        {
            ...
            foreach (IFormFile photo in productVM.Photos ?? new List<IFormFile>())
            {
                ...
                {
                    TempData["Message"] += $"<p class=\"text-danger\">{photo.FileName} adli file tipi uygun deyil</p>";
                    continue;
                }
                if (!photo.ValidateSize(600))
                {
                    TempData["Message"] += $"<p class=\"text-danger\">{photo.FileName} adli file olcusu uygun deyil</p>";
                    continue;
                }
